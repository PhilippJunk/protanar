...
.stat_analysis_limma <- function(mx,
                                 groups,
                                 contrasts) {
...
#'   each row corresponds to a contrast to test.
#' @return A data frame with raw pvalues.
.stat_analysis_ttest <- function(mx,
                                 groups,
                                 contrasts) {
  # for each contrast
  contrasts %>%
...
construct_contrasts_control <- function(p_df, cntrl_group) {
  ...
  tibble::tibble(
    a = all_groups[!all_groups == cntrl_group],
    b = cntrl_group
  )
}
...
diff_type <- function(p_df, contrasts) {
  ...
  contrasts %>%
      ...
      p_df %>%
        ...
        dplyr::summarise(
          diff_type = dplyr::case_when(
            all(c(a, b) %in% group) ~ "value_value",
    ...
    dplyr::bind_rows() %>%
    tidyr::pivot_wider(
      names_from = contrast, values_from = diff_type,
